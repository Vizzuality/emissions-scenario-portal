WITH duplicated_team_indicators AS (
  SELECT category, subcategory, name, min_id, dup_id FROM (
    SELECT category, subcategory, name, min_id, UNNEST(duplicated_ids) AS dup_id FROM (
      SELECT category, subcategory, name, MIN(id) AS min_id, ARRAY_AGG(id) AS duplicated_ids, COUNT(*)
      FROM indicators
      WHERE parent_id IS NULL AND model_id IS NOT NULL
